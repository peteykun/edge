...
(function() {
  ...
  this.refresh_active_contact = function() {
    if ($('.contacts').length === 1) {
      return;
    }
    $('.contacts').hide();
    return $('#cc_' + $('#message_contact_category_id option:selected').val()).show();
  };

  ready = function() {
    refresh_active_contact();
    return $('#container').on({
      change: refresh_active_contact
    }, "#message_contact_category_id");
  };
